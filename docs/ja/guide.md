...
6. [ダッシュボード（vx3 dash）](#ダッシュボードvx3-dash)
7. [設定ファイル（vx.config.json）](#設定ファイルvxconfigjson)
8. [SDK API リファレンス](#sdk-api-リファレンス)
9. [Payment モジュール](#payment-モジュール)
10. [React 支払いコンポーネント](#react-支払いコンポーネント)
11. [IPFS モジュール](#ipfs-モジュール)
12. [Hardhat 連携](#hardhat-連携)
...
```powershell
vx3 dash --open
# ブラウザで http://127.0.0.1:4000 が自動で開く
```
...
ローカル開発 HTTP サーバを起動します（ポート 3000）。

```powershell
vx3 node
vx3 node --port 3001
vx3 node --debug    # /debug エンドポイントを有効化
```
...
| `http://localhost:3000/events` | SSE ストリーム（リアルタイムブロック更新） |
| `http://localhost:3000/debug` | デバッグ UI（`--debug` 時のみ） |

---

### `vx3 dash`

リアルタイム開発ダッシュボードを起動します（ポート 4000）。
...
- ライブブロック番号（SSE によるリアルタイム更新）
- EIP-1559 ガス料金表示（gasPrice / maxFeePerGas / baseFee）
- `vx.config.json` から読み込んだ RPC エンドポイント一覧（コピーボタン付き）
- アクティビティログ（ブロック・ガス・エラーをタイムスタンプ付きで記録）
- SDK コマンドリファレンスをサイドバーに常時表示
...
| `/api/gas` | GET | EIP-1559 ガス料金データ（JSON） |
...
`vx3 dash` は、ローカル開発時に使用するリアルタイム Web UI ダッシュボードです。
`vx.config.json` で設定した RPC に接続し、ブロックチェーンの状態を視覚的に確認できます。

### 起動方法

```powershell
# デフォルト（http://127.0.0.1:4000）
vx3 dash

# ポートとホストを指定
vx3 dash --port 5000 --host 0.0.0.0
...
```
┌─ ヘッダー ──────────────────────────────────────────────────┐
│ VX3  v0.0.2        ● live    status  block  gas  rpc       │
├─ サイドバー ──────┬─ メインコンテンツ ────────────────────────┤
│ サーバーアドレス   │ [Block]   [Gas Price] [Base Fee] [RPC数] │
│ RPC URL           ├──────────────────────────────────────────┤
│ コマンド一覧      │ ガス料金テーブル   │ アクティビティログ     │
│ エンドポイント一覧 ├──────────────────────────────────────────┤
│ ドキュメントリンク │ RPC エンドポイント一覧（コピーボタン付き） │
└───────────────────┴──────────────────────────────────────────┘
```

### SSE によるリアルタイム更新

ダッシュボードは `/events` エンドポイント（Server-Sent Events）で以下を 4 秒ごとに受信します：

| イベント | データ | 説明 |
| :--- | :--- | :--- |
| `block` | `{ blockNumber: number }` | 最新ブロック番号 |
| `gas` | `GasFees` オブジェクト | 最新のガス料金データ |
...
# 最新ブロック番号取得
curl http://127.0.0.1:4000/api/block

# ガス料金取得
curl http://127.0.0.1:4000/api/gas

# RPC 設定一覧
curl http://127.0.0.1:4000/api/rpc
```

> [!NOTE]
> `vx3 node`（ポート 3000）と `vx3 dash`（ポート 4000）は独立したサーバーです。
> 同時に起動して並行利用できます。
...
*このドキュメントは `@vx3/vx` SDK の日本語詳細ガイドです。英語 / 日本語の概要は [README.md](../../README.md) を参照してください。*
