...
function handleSaveCommand(args: string[]): void {
  if (args.length < 4) {
    console.error(
      'Usage: vx3 rpc save <filename> <host> <port> <protocol> [rpcsDir]'
    );
...
function handleAddCommand(args: string[]): void {
  if (args.length < 4) {
    console.error(
      'Usage: vx3 rpc add <filename> <host> <port> <protocol> [rpcsDir]'
    );
...
function showRpcHelp(): void {
  ...
  console.log(
    '  save <filename> <host> <port> <protocol> [dir]  - Save new RPC configuration'
  );
  console.log(
    '  save-ipfs <filename> <gateway|apiHost> [apiPort apiProtocol] [dir] - Save IPFS gateway or API entry'
  );
  console.log(
    '  load <filename> [dir]                           - Load and display RPC configuration'
  );
  console.log(
    '  list                                            - List all saved configurations'
  );
  console.log(
    '  create [filename] [dir]                         - Create default configuration'
  );
  console.log(
    '  add <filename> <host> <port> <protocol> [dir]   - Add endpoint to existing config'
  );
  console.log(
    '  add-ipfs <filename> <gateway|apiHost> [apiPort apiProtocol] [dir] - Add IPFS endpoint to existing config'
  );
  console.log(
  ...
  console.log(
    '  filename   - Configuration file name (without .json extension)'
  );
...
function handleSaveIpfsCommand(args: string[]): void {
  ...
  if (args.length < 2) {
    ...
    console.error(
      'Example (gateway): vx3 rpc save-ipfs localhost https://ipfs.io'
    );
    console.error(
      'Example (api): vx3 rpc save-ipfs localhost ipfs.example.com 5001 https'
    );
    process.exit(1);
  }
